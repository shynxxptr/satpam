...
class SatpamBotInstance:
    ...
    def setup_commands(self):
        ...
        async def panggil_satpam(interaction: discord.Interaction, channel: Optional[discord.VoiceChannel] = None):
            ...
            if self.voice_client and self.voice_client.is_connected():
                # Jika bot sedang di idle channel, bisa langsung pindah
                if self.is_idle:
                    ...
                    # Bot sedang menjaga channel lain
    ...
    async def _check_and_assign_queue(self):
        """Check queue dan auto-assign bot ke user berikutnya"""
        try:
            ...
            # Get channel
            channel = self.bot.get_channel(next_user["channel_id"])
            if not channel:
                # Try fetch
                try:
                    channel = await self.bot.fetch_channel(next_user["channel_id"])
                except:
            ...
            try:
                ...
                # Send notification
                await notification_manager.send_queue_ready_notification(channel, user, self.bot_number)
                await notification_manager.send_join_notification(channel, user, self.bot_number, tier_info['name'], stay_duration_hours)
    ...
    async def _auto_reconnect_task(self):
        ...
        while True:
            try:
                ...
                if self.current_channel and not self.is_idle:
                    if not self.voice_client or not self.voice_client.is_connected():
                        # Bot disconnected, try reconnect
    ...
    async def _scheduled_stay_task(self):
        """Task untuk handle scheduled stays"""
        while True:
            try:
                ...
                for schedule in active_schedules:
                    ...
                    if 0 <= time_diff <= 60:  # Within 1 minute
                        ...
                        try:
                            ...
                            if channel.id not in shared_assignments:
                                # Auto-assign bot to channel
                                if self.is_idle or (not self.voice_client or not self.voice_client.is_connected()):
                                    # Bot is available, assign
                                    tier = subscription_manager.get_user_tier(user)
                                    stay_duration_hours = subscription_manager.get_stay_duration_hours(user)
                                    ...
                                    tier_info = subscription_manager.get_tier_info(tier)
                                    await notification_manager.send_join_notification(
                                        channel, user, self.bot_number, tier_info['name'], stay_duration_hours
                                    )
